...
// 4. Email admin APRÈS génération (avec vrais résultats v0 + Claude)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ==========================================
    // 1. RÉPONSE IMMÉDIATE — évite timeout client
    // ==========================================
    res.status(200).json({ success: true, processing: true });
 
    // ==========================================
    // 2. TOUT EN PARALLÈLE — maximise le temps dispo (10s Vercel Hobby)
    //    • Notion save
    //    • Email confirmation prospect
    //    • Génération Claude (site HTML complet, pièce jointe)
    //    • Génération v0 (lien preview React)
    // ==========================================
    const prompt = generateSitePrompt(data);
    console.log('[WebPrestige] Lancement en parallèle : Notion + emails + v0 + Claude...');
 
    const [notionResult, prospectEmailResult, claudeResult, v0Result] = await Promise.allSettled([
      saveToNotion(data),
      data.email ? sendConfirmationEmail(data) : Promise.resolve(null),
      generateWithClaude(data, prompt),
      triggerV0(data, prompt),
    ]);
 
    const notionPageId = notionResult.status === 'fulfilled' ? notionResult.value?.id : null;
    const emailId = prospectEmailResult.status === 'fulfilled' ? prospectEmailResult.value : null;
    ...
    if (notionPageId) console.log('[Notion] ✅ Sauvegardé:', notionPageId);
    else console.error('[Notion] ❌', notionResult.reason?.message);
 
    if (emailId) console.log('[Resend] ✅ Email prospect envoyé:', emailId);
    else if (data.email) console.error('[Resend] ❌', prospectEmailResult.reason?.message);
 
    if (claudeHtml) console.log('[Claude] ✅ HTML généré:', claudeHtml.length, 'chars');
    else console.error('[Claude] ❌', claudeResult.reason?.message);
 
    if (v0Url) console.log('[v0] ✅ URL:', v0Url);
    else console.error('[v0] ❌', v0Result.reason?.message);
